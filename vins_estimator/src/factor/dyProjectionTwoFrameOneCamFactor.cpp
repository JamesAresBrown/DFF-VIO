...
#include "dyProjectionTwoFrameOneCamFactor.h"
#include "projectionTwoFrameOneCamFactor.h"

Eigen::Matrix2d DyProjectionTwoFrameOneCamFactor::sqrt_info;
double DyProjectionTwoFrameOneCamFactor::sum_t;

DyProjectionTwoFrameOneCamFactor::DyProjectionTwoFrameOneCamFactor(const Eigen::Vector3d &_pts_i, const Eigen::Vector3d &_pts_j,
...
bool DyProjectionTwoFrameOneCamFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
    TicToc tic_toc;
    ...
    Eigen::Vector3d mc(parameters[5][0], parameters[5][1], 0);
//    Eigen::Vector3d mc(0, 0, 0);


//    double dtt = t_j - t_i;
//    cout << "opt:                    " << t_i << "    " << t_j << endl;
//    dt = (dt < 0.03 || dt >=1 )?0:dt;
//    cout << dt << endl;

    Eigen::Vector3d pts_i_td, pts_j_td;
    pts_i_td = pts_i - (td - td_i) * velocity_i + mc;
    ...
    if (jacobians)
    {
        ...
        if (jacobians[5])
        {
            Eigen::Map<Eigen::Matrix<double, 2, 2, Eigen::RowMajor>> jacobian_mc(jacobians[5]);
            Eigen::Matrix<double, 3, 3> jaco_mc;
            jaco_mc = ric.transpose() * Rj.transpose() * Ri * ric * (1.0 / inv_dep_i);
            jacobian_mc = reduce * jaco_mc.leftCols<2>();
//            jacobian_mc = (dt * reduce).leftCols<2>();
        }
    }
    sum_t += tic_toc.toc();
...
void DyProjectionTwoFrameOneCamFactor::check(double **parameters)
{
